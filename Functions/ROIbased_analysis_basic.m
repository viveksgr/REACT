function [Tmat] = ROIbased_analysis_basic(atlas_dir,aal_file,aal_labels,opt)
    ...
    neurotransmitter_dirs = dir(fullfile(atlas_dir, 'rNormalized*.nii'));
    ...
    for n_idx = 1:length(neurotransmitter_dirs)
        neurotransmitter_file = fullfile(neurotransmitter_dirs(n_idx).folder, neurotransmitter_dirs(n_idx).name);
        fprintf('Processing neurotransmitter: %s\n', neurotransmitter_dirs(n_idx).name);
        
        
        file_tmap = spm_read_vols(spm_vol(          neurotransmitter_file));
        Tmat(n_idx,:) = compute_combined_roi_t_scores( file_tmap ,   aal_masks,'mean');
